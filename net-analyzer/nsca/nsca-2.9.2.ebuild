...
RDEPEND="${DEPEND}
	!minimal? ( || ( net-analyzer/nagios net-analyzer/icinga ) )
	sys-apps/openrc"
...
src_prepare() {
	epatch "${FILESDIR}"/${P}-small-warning-cleanup.patch
}

src_configure() {
	append-cflags -D_POSIX_SOURCE -D_GNU_SOURCE
...
src_install() {
	...
	dobin src/send_nsca "${FILESDIR}"/send_host_check
...
pkg_postinst() {
	...
	elog ""
	elog "A sample send_check wrapper script has been installed in /usr/bin."
	elog "To use it, set the MON_HOST to your monitoring data collector host"
	elog "and choose a command that you want to use for a simple host check."
}
